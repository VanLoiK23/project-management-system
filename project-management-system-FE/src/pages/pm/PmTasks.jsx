...
function normalizeTask(task) {
  ...
  return {
    ...
    // Backend có thể trả name hoặc title
...
export default function PmTasks() {
    ...
    try {
      ...
      if (editingTask) {
        ...
        const updatedTask = normalizeTask({
          ...
          status: updatedData?.status ?? editingTask.status ?? "TODO",
      ...
      // =====================================================
        ...
        if (!createdData?.id) {
          // Backend thành công nhưng response không có task
          // thì reload từ server để lấy task thật.
          await loadTasks();
        } else {
          const fallbackAssignees = members.filter((member) =>
            payload.assigneeIds.includes(Number(member.id)),
  ...
  async function updateStatus(task, status) {
    ...
    try {
      const response = await axios.put(`/tasks/${task.id}/status`, {
        status,
      });
  ...
  async function loadComments(task) {
    ...
    try {
      ...
      /*
       * Không đóng detail nếu load comment lỗi.
  ...
  async function handleAddComment() {
    ...
    try {
      ...
      if (newComment) {
        ...
        // Nếu backend không trả data comment,
        // reload comment từ server.
  ...
  async function openDetail(task) {
    ...
    // Controller đã có GET /tasks/{id}/comments
