...
/**
 * ====== ĐÃ KHỚP VỚI BACKEND (DocumentServiceImpl đã cập nhật) ======
 * 1. GET /documents/project/{projectId}?folderId=... trả về danh sách tài liệu
 *    đã lọc theo quyền của user hiện tại. Mỗi document có sẵn 2 field server tính:
 *      - canDownload: boolean
 *      - canEdit: boolean (PM/Admin, chủ sở hữu tài liệu, hoặc được cấp quyền EDIT)
 *    -> Member được phép upload tài liệu MỚI vào bất kỳ thư mục có sẵn/Kho gốc nào
 *       (chỉ cần là thành viên dự án); nhưng CHỈ được sửa/khôi phục version của
 *       tài liệu do chính mình upload, trừ khi PM cấp thêm quyền EDIT.
 * 2. Endpoint GET /tasks/project/{projectId}/my-tasks (task được giao cho user hiện tại)
 *    dùng để cho phép Member gắn tài liệu upload vào 1 task cụ thể (coi như deliverable).
 *    Đây là optional - nếu API chưa có, field "Liên kết công việc" sẽ tự ẩn (không lỗi).
 * 3. Không có action Tạo/Xóa thư mục, Đổi tên, Di chuyển, Xóa tài liệu, Phân quyền,
 *    Đổi Công khai/Bảo mật trong trang này - các chức năng đó giữ nguyên PM/Admin-only.
 */
...
const apiFetchFolders = (projectId) =>
  axios.get(`/documents/folders/project/${projectId}`).then((res) => res.data);
...
// Optional - tự ẩn field nếu API chưa tồn tại (404/500 đều bị nuốt lỗi ở nơi gọi)
const apiFetchMyTasks = (projectId) =>
  axios.get(`/member/tasks/${projectId}/my-tasks`).then((res) => res.data);
...
function getFileIcon(name = "", className = "h-5 w-5") {
  ...
  if (
    ["js", "jsx", "ts", "tsx", "java", "kt", "py", "html", "css", "json", "sql", "xml"].includes(ext)
  ) {
...
/**
 * Quyền của Member hiện tại trên 1 document - lấy trực tiếp từ field server
 * tính sẵn (canDownload/canEdit trong DocumentResponse). Fallback theo visibility
 * chỉ áp dụng nếu 2 field này vô tình bị thiếu (API cũ chưa deploy kịp).
 */
function getMyAccess(doc) {
  const isPublic = String(doc.visibility || "PUBLIC").toUpperCase() === "PUBLIC";

  const canDownload =
    typeof doc.canDownload === "boolean" ? doc.canDownload : isPublic;

  const canEdit = typeof doc.canEdit === "boolean" ? doc.canEdit : false;

  return { canView: canDownload || canEdit, canDownload, canEdit };
}
...
function buildFolderTree(folders) {
  ...
  folders.forEach((folder) => {
    map.set(String(folder.id), { ...folder, children: [] });
  });
  ...
  folders.forEach((folder) => {
    ...
    if (folder.parentId && map.has(String(folder.parentId))) {
      map.get(String(folder.parentId)).children.push(current);
    } else {
      roots.push(current);
    }
  });
...
function FolderTreeItem({ folder, level, selectedFolderId, onSelect }) {
  const [expanded, setExpanded] = useState(true);
  const hasChildren = folder.children?.length > 0;

  const selected =
    selectedFolderId !== null && String(selectedFolderId) === String(folder.id);
      ...
      >
        <button
          ...
          className={`flex h-6 w-6 shrink-0 items-center justify-center rounded ${
            hasChildren ? "hover:bg-slate-200" : "cursor-default"
          }`}
        >
          {hasChildren &&
        ...
        >
          ...
          <span className="ml-auto text-xs text-slate-400">{folder.documentCount || 0}</span>
        </button>
      </div>
...
export default function MemberDocuments() {
  ...
  const [myTasks, setMyTasks] = useState([]); // optional - ẩn field nếu rỗng
  ...
  const loadDocuments = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await apiFetchDocuments(projectId, selectedFolderId);
  ...
  const loadMyTasks = useCallback(async () => {
    if (!projectId) return;
    try {
      const data = await apiFetchMyTasks(projectId);
      setMyTasks(extractList(data));
    } catch {
      // API optional - im lặng bỏ qua, field liên kết công việc sẽ tự ẩn
      setMyTasks([]);
    }
  }, [projectId]);
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return documents.filter((doc) => {
      return (
        ...
        String(doc.uploadedByName || "").toLowerCase().includes(query) ||
        String(doc.folderName || "").toLowerCase().includes(query)
      );
    });
  }, [documents, searchQuery]);

  const stats = useMemo(() => {
    const editable = documents.filter((doc) => getMyAccess(doc).canEdit).length;
    return { documents: documents.length, folders: folders.length, editable };
  }, [documents, folders]);
  ...
  const openUploadModal = () => {
    ...
    setDocDesc("");
    setUploadTaskId("");
    setUploadFolderId(selectedFolderId ? String(selectedFolderId) : "");
  ...
  const handleUploadSubmit = async (event) => {
    ...
    if (uploadTaskId) formData.append("taskId", uploadTaskId);

    formData.append("name", docName.trim() || uploadFile.name);
    ...
    try {
      ...
      showToast("error", getApiError(err, "Không thể tải tài liệu lên."));
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleNewVersionSubmit = async (event) => {
    ...
    if (!selectedDoc || !versionFile) {
      showToast("error", "Vui lòng chọn file cho phiên bản mới.");
      return;
    }
    ...
    const formData = new FormData();
    formData.append("changeDescription", changeDesc.trim());
    formData.append("file", versionFile);

    try {
      await apiUploadVersion(selectedDoc.id, formData);
      ...
      await loadDocuments();
      showToast("success", "Tạo phiên bản mới thành công.");
    } catch (err) {
      showToast("error", getApiError(err, "Không thể tạo phiên bản mới."));
    } finally {
  ...
  const openHistoryModal = async (doc) => {
    ...
    try {
      ...
      showToast("error", getApiError(err, "Không thể tải lịch sử phiên bản."));
    } finally {
  ...
  const handleRestoreVersion = async (version) => {
    ...
    const confirmed = window.confirm(`Khôi phục phiên bản v${version.versionNumber}?`);
    if (!confirmed) return;

    setRestoringVersionId(version.id);

    try {
      await apiRestoreVersion(selectedDoc.id, version.id);

      const data = await apiFetchVersions(selectedDoc.id);
      setVersions(extractList(data));

      await loadDocuments();
      showToast("success", `Đã khôi phục phiên bản v${version.versionNumber}.`);
    } catch (err) {
      showToast("error", getApiError(err, "Không thể khôi phục phiên bản."));
    } finally {
      setRestoringVersionId(null);
    }
  };

  const handleDownload = async (versionId, fileName = "document") => {
    try {
      const response = await axios.get(`/documents/versions/${versionId}/download`, {
        responseType: "blob",
      });
      ...
      showToast("error", getApiError(err, "Không thể tải file."));
    }
  };
    ...
    <div className="min-h-screen space-y-6 bg-slate-50 pb-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Xem, tải xuống và cập nhật các tài liệu bạn được cấp quyền trong dự án.
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-500">Tài liệu bạn xem được</p>
              <p className="mt-1 text-2xl font-bold text-slate-900">{stats.documents}</p>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-500">Được phép chỉnh sửa</p>
              <p className="mt-1 text-2xl font-bold text-slate-900">{stats.editable}</p>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-500">Thư mục</p>
              <p className="mt-1 text-2xl font-bold text-slate-900">{stats.folders}</p>
            </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="font-semibold text-slate-900">Kho tài liệu</h2>
        ...
        <div className="grid min-h-[560px] grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="border-b border-slate-200 bg-slate-50/70 p-4 lg:border-b-0 lg:border-r">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-800">Thư mục</h3>
              {loadingFolders && <Loader2 className="h-4 w-4 animate-spin text-slate-400" />}
            </div>
            ...
            {folders.length === 0 && !loadingFolders && (
              <div className="mt-5 rounded-xl border border-dashed border-slate-300 p-4 text-center">
                ...
                <p className="mt-2 text-xs text-slate-500">Chưa có thư mục.</p>
              </div>
            )}
          </aside>
            ...
            {loading ? (
              <div className="flex min-h-[480px] items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="mt-3 text-sm text-slate-500">Đang tải tài liệu...</p>
                </div>
              </div>
                ...
                <table className="w-full min-w-[820px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredDocuments.map((doc) => {
                      ...
                      const fileName = doc.currentFileName || doc.name || "document";
                      const access = getMyAccess(doc);
                        ...
                        <tr key={doc.id} className="group transition hover:bg-slate-50">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <p className="truncate font-semibold text-slate-800" title={doc.name}>
                                  {doc.name}
                                </p>
                                <div className="mt-1 flex items-center gap-2 text-xs text-slate-400">
                                  <span>{formatBytes(doc.currentFileSize)}</span>
                                  <span>•</span>
                                  <span>
                          ...
                          <td className="px-4 py-4">
                            ...
                            <p className="mt-0.5 truncate text-xs text-slate-400">
                              {doc.uploadedByName || "Không xác định"}
                            </p>
                          </td>
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center justify-end gap-1">
                              <button
                                ...
                                onClick={() => handleDownload(doc.currentVersionId, fileName)}
                                title={access.canDownload ? "Tải xuống" : "Bạn chưa được cấp quyền tải"}
                                className="rounded-lg p-2 text-slate-500 transition hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent"
                              >
                              ...
                              <button
                                ...
                                className="rounded-lg p-2 text-slate-500 transition hover:bg-amber-50 hover:text-amber-600 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent"
                              >
                              ...
                              <button
                                ...
                                className="rounded-lg p-2 text-slate-500 transition hover:bg-violet-50 hover:text-violet-600 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent"
                              >
                                <UploadCloud className="h-4 w-4" />
                              </button>
                            </div>
      ...
      {showUploadModal && (
        <Modal title="Tải tài liệu lên" onClose={() => !uploading && setShowUploadModal(false)}>
          <form onSubmit={handleUploadSubmit} className="space-y-5 p-6">
            <div>
              <label className="mb-2 block text-sm font-semibold text-slate-700">File tài liệu</label>
              <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center transition hover:border-blue-400 hover:bg-blue-50/40">
                ...
                <span className="mt-3 text-sm font-semibold text-slate-700">
                  {uploadFile ? uploadFile.name : "Chọn file để tải lên"}
                </span>
                <span className="mt-1 text-xs text-slate-400">
                  Đây sẽ là tài liệu mới trong kho tài liệu dự án.
                </span>
                <input
                  type="file"
                  className="hidden"
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ví dụ: Báo cáo kết quả công việc"
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold text-slate-700">Thư mục</label>
              <select
            ...
            {myTasks.length > 0 && (
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-700">
                  Liên kết với công việc <span className="font-normal text-slate-400">(tùy chọn)</span>
                </label>
                <select
                  value={uploadTaskId}
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Đánh dấu tài liệu này là kết quả (deliverable) của công việc bạn chọn.
                </p>
              </div>
            )}
      ...
      {showVersionModal && selectedDoc && (
        <Modal
          ...
          onClose={() => !submittingVersion && setShowVersionModal(false)}
        >
          <form onSubmit={handleNewVersionSubmit} className="space-y-5 p-6">
            <div className="flex items-center gap-3 rounded-xl bg-blue-50 p-4">
              <History className="h-6 w-6 text-blue-600" />
              <div>
                <p className="text-sm font-semibold text-blue-900">
                  Phiên bản hiện tại: v{selectedDoc.currentVersionNumber || 1}
                </p>
                <p className="mt-1 text-xs text-blue-700">
                  File mới sẽ trở thành phiên bản hiện tại.
                </p>
              </div>
            </div>
              ...
              <label className="flex cursor-pointer items-center gap-4 rounded-xl border-2 border-dashed border-slate-300 p-5 transition hover:border-blue-400 hover:bg-blue-50/30">
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-700">
                    {versionFile ? versionFile.name : "Chọn file phiên bản mới"}
                  </p>
                  <p className="mt-1 text-xs text-slate-400">Chọn file mới để tạo version.</p>
                </div>
                <input
                  type="file"
                  className="hidden"
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold text-slate-700">Mô tả thay đổi</label>
              <textarea
                value={changeDesc}
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 pt-4">
              ...
              <button
                type="submit"
                disabled={submittingVersion || !versionFile}
        ...
        >
          <div className="p-6">
            <div className="mb-5 flex items-center justify-between rounded-xl bg-slate-50 p-4">
              <div className="flex items-center gap-3">
                {getFileIcon(selectedDoc.currentFileName || selectedDoc.name, "h-7 w-7")}
                <div>
                  <p className="font-semibold text-slate-800">{selectedDoc.name}</p>
              ...
              {getMyAccess(selectedDoc).canEdit && (
                <button
                  type="button"
            ...
            {loadingVersions ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 p-10 text-center">
                ...
                <p className="mt-2 text-sm text-slate-500">Chưa có lịch sử phiên bản.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {versions.map((version) => {
                  const isCurrent = Boolean(version.current);
                  const fileName = version.fileName || selectedDoc.name;

                  return (
                    <div
                      key={version.id}
                      className={`rounded-xl border p-4 transition ${
                        isCurrent ? "border-blue-200 bg-blue-50/40" : "border-slate-200 bg-white"
                      }`}
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex min-w-0 items-start gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500">
                              ...
                              <span>{version.editedByName || "Không xác định"}</span>
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          <button
                            type="button"
                            disabled={!getMyAccess(selectedDoc).canDownload}
                            onClick={() => handleDownload(version.id, fileName)}
                            className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                          >
                            <Download className="h-4 w-4" />
                            Tải
                          </button>

                          {!isCurrent && getMyAccess(selectedDoc).canEdit && (
                            <button
                              type="button"
